...
public class LoadXMLToDocumentStageTest {
	...
	@Test
	public void loadXMLStageTest() throws FileNotFoundException, IOException {
		File xmlFile = createExampleXMLFile();

		LoadXMLToDocumentStage stage = new LoadXMLToDocumentStage();
		List<Document> documents = new ArrayList<Document>();

		StageTester.test(stage).and().send(xmlFile.getAbsolutePath()).to(stage.getInputPort()).and().receive(documents)
	...
	private File createExampleXMLFile() throws IOException, FileNotFoundException {
		...
		OutputStream outputStream = Files.newOutputStream(xmlFile.toPath());
